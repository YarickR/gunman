...
Shader "FX/_Psycho/Puddle" {
    ...
    SubShader {
        ...
        Pass {
			...
			{
				v2f o;
				float distance = dot(i.vertex.xyz, i.vertex.xyz);
				i.vertex.z += (sin((i.color.a + 0.5f) * _Friq + distance) + 1.0f) * _Amplitude * saturate( 1.0f * _PuddleRadius - distance);
			...
			{
				fixed textureColor = tex2D(_MainTex, i.uv_textureMask.xy).r;

				textureColor *= i.uv_textureMask.z;

				fixed4 color = lerp(_Color, _Color2, textureColor.rrrr);
				color *= i.color;
