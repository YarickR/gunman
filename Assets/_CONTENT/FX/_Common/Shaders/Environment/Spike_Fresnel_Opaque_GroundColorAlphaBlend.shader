...
Shader "FX/_Environment/Spike_Fresnel_Opaque_GroundColorAlphaBlend" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
			...
			fixed4 frag(v2f i) : COLOR
			{
				...
				color *= i.color;
				float groundColorCoef = saturate(abs(i.uv_GlabalY.z - _GroundColorHeight) / _GroundColorThickness);
				float3 groundColor = _GroundColor * _GroundColorMul;
				color = lerp(groundColor, color, groundColorCoef);
