...
#include "UnityCG.cginc"
	...
	fixed4 frag(v2f i) : COLOR
	{
		...
		fixed eraseFactor = saturate( (eraseMask - _EraseProgress) * MAX_FLOAT);//замена потомучто step(_EraseProgress, eraseMask) не пашет на Sony Xperia Z3;
		clip(eraseFactor - 0.5);
		
		fixed eraseProgressPow = pow(_EraseProgress, 0.5f);
		fixed4 eraseColor = _EraseColor * smoothstep(eraseProgressPow + _EraseBorderWidth, eraseProgressPow, eraseMask);
...
#pragma target 2.0
	...
	float4 frag(VertexOutput i) : COLOR{
		...
		return _GlobalShadowColor + _GlobalShadowColor.a + _EraseProgress/4;
